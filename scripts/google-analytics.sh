...
for candidate in python3 python py; do
    if command -v "$candidate" >/dev/null 2>&1 && "$candidate" -I -X utf8 -c "$PROBE" >/dev/null 2>&1; then
        exec "$candidate" -I -X utf8 "$ENTRYPOINT" "$@"
    fi
done

printf '%s\n' '{"schemaVersion":1,"cliVersion":"0.4.3","ok":false,"command":"bootstrap","status":"error","data":{},"warnings":[],"errors":[{"code":"PYTHON_RUNTIME_UNAVAILABLE","message":"Supported CPython 3.10-3.13 was not found.","retryable":false,"details":{},"nextAction":"Install a standard 64-bit CPython from https://www.python.org/downloads/ after explicit consent, then run again."}]}'
exit 2
